...
def build_messages(image, prompt: str):
    # For Kimi-VL, the chat template only needs to know there is one image slot.
    # The real image tensor is passed separately to `processor(images=..., text=...)`.
    return [
        {
            "role": "user",
            "content": [
...
def run_single_inference(model, processor, image, prompt, max_new_tokens=1024):
    ...
    try:
        ...
        # Fallback in case the remote processor does not accept a PIL object
        # inside the chat template and only uses the multimodal slot marker.
        fallback_messages = build_messages(image="<image>", prompt=prompt)
        text = processor.apply_chat_template(
            fallback_messages,
            add_generation_prompt=True,
            return_tensors="pt",
...
def load_model_and_processor(model_name: str, use_flash_attn: bool):
    model_kwargs = get_model_kwargs(use_flash_attn)
    ...
    try:
        ...
        if not use_flash_attn:
            raise
        print("Warning: flash_attention_2 is not supported by this model loader. Retrying without it.")
        fallback_kwargs = {
            "torch_dtype": "auto",
            "device_map": "auto",
            "trust_remote_code": True,
...
def parse_args():
    ...
    parser.add_argument("--model_name", type=str, default="moonshotai/Kimi-VL-A3B-Instruct")
